...
const TopNavbar = () => {
  ...
  if (user?.role === "admin") {
    ...
    menuItems.push({ path: "/mortgage", label: "Mortgage", icon: FiBriefcase }); // ✅ Add Mortgage
  }
          ...
          <div className="flex items-center gap-1">
            {menuItems.map((item) => (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center px-3 py-2 rounded-md text-sm font-medium transition whitespace-nowrap ${
                  isActive(item.path)
